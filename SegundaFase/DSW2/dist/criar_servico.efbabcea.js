document.addEventListener('DOMContentLoaded', ()=>{
    ...
    async function carregarTarefa(id) {
        try {
            ...
            if (tarefa) preencherFormulario(tarefa);
            else {
    ...
    function preencherFormulario(tarefa) {
        document.getElementById('tipo').value = tarefa.tipo === "Oferta de Servi\xe7o" ? 'oferta' : 'solicitacao';
    ...
    async function salvarTarefa(tarefa) {
        try {
            ...
            if (response.ok) {
                ...
                window.location.href = `home.html?id=${userId}`;
            } else throw new Error('Erro ao salvar tarefa');
        } catch (error) {
    ...
    formServico.addEventListener('submit', async (event)=>{
        ...
        const tarefa = {
            tipo: document.getElementById('tipo').value === 'oferta' ? "Oferta de Servi\xe7o" : "Solicita\xe7\xe3o de Servi\xe7o",
    ...
    // Inicialização
    if (tarefaId) // Se houver um ID de tarefa, carrega os dados para edição
    carregarTarefa(tarefaId);
    else if (tipo) // Se houver um tipo de serviço, define o valor padrão no formulário
    document.getElementById('tipo').value = tipo;
    else {
        alert("Tipo de servi\xe7o n\xe3o especificado.");
        window.location.href = `home.html?id=${userId}`;
    }
});

//# sourceMappingURL=criar_servico.efbabcea.js.map
